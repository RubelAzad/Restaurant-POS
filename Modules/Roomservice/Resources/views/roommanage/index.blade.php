                    ...
                    <form id="form-filter">
                        <div class="row">
                            <div class="form-group col-md-3">
                                <label for="name">roommanage Name</label>
                                <input type="text" class="form-control" name="name" id="name" placeholder="Enter roommanage name">
                            </div>
...
@include('roomservice::roommanage.modal')
@endsection
...
$(document).ready(function(){
    ...
    table = $('#dataTable').DataTable({
        ...
        "ajax": {
            "url": "{{route('roommanage.datatable.data')}}",
            "type": "POST",
            "data": function (data) {
        ...
        "buttons": [
            @if (permission('roommanage-report'))
            {
    ...
    $(document).on('click', '#save-btn', function () {
        ...
        let url = "{{route('roommanage.store.or.update')}}";
        ...
        if (id) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#store_or_update_form #room_service_extra_price').val(data.room_service_extra_price);
                    ...
                    $('#store_or_update_modal .modal-title').html(
                        '<i class="fas fa-edit"></i> <span>Edit ' + data.id + '</span>');
    ...
    $(document).on('click', '.delete_data', function () {
        ...
        let url   = "{{ route('roommanage.delete') }}";
    ...
    function multi_delete(){
        ...
        if(ids.length == 0){
            ...
            let url = "{{route('roommanage.bulk.delete')}}";
    ...
    $(document).on('click', '.change_status', function () {
        ...
        let name  = $(this).data('name');
        let row   = table.row($(this).parent('tr'));
        let url   = "{{ route('roommanage.change.status') }}";
...
$('#type_id').on('change',function(){
        ...
        {
            $('.roommanage_floor').removeClass('d-none');
            
        }else{
            $('.roommanage_floor').addClass('d-none');
        }
