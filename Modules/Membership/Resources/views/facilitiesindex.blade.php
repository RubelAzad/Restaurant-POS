...
@section('content')
        ...
        <div class="row">
            ...
            <div class="col-xl-12">
                ...
                <div class="dt-entry__header">
                    ...
                    <!-- /entry heading -->
                    @if (permission('facilitysettings-add'))
                        <button class="btn btn-primary btn-sm"
                            onclick="showStoreFormModal('Add New facilitysettings','Save')">
                ...
                <!-- facilitysettings -->
                <div class="dt-facilitysettings">

                    <!-- facilitysettings Body -->
                    <div class="dt-facilitysettings__body">
    ...
    @include('membership::facilitiesmodal')
@endsection
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            table = $('#dataTable').DataTable({
                ...
                "ajax": {
                    "url": "{{ route('facilitysettings.datatable.data') }}",
                    "type": "POST",
                    "data": function(data) {
                ...
                "buttons": [
                    @if (permission('facilitysettings-report'))
                        {
                            'extend': 'colvis',
                            ...
                            'className': 'btn btn-secondary btn-sm text-white',
                            "title": "Menu List",
                            "filename": "facilitysettings-list",
                            "exportOptions": {
                                columns: function(index, data, node) {
                                    return table.column(index).visible();
                                }
            ...
            $(document).on('click', '#save-btn', function() {
                ...
                let url = "{{ route('facilitysettings.store.or.update') }}";
            ...
            $(document).on('click', '.edit_data', function() {
                ...
                if (id) {
                    $.ajax({
                        url: "{{ route('facilitysettings.edit') }}",
                        type: "POST",
                        data: {
                            id: id,
                            _token: _token
                        ...
                        success: function(data) {
                            ...
                            $('#store_or_update_form #facilities_id').val(data.facilities_id);
                            $('#store_or_update_form #facilities_price').val(data
                                .facilities_price);
                            // $('#store_or_update_form #hours').prop('checked' ,data.facilities_status);
                            //$('input[name="facilities_status"]:checked').val('');
                            //$('#store_or_update_form #facilitysettings_facilities_id').val(data.facilitysettings_facilities_id);
                            //$('#store_or_update_form #ri_menu').val(data.ri_menu);

                            let facilitiesStatusValue = data.facilities_status;
                            $('.facilitiesRadio').each(function() {
                                if ( $(this).val() === facilitiesStatusValue) {
                            ...
                            $('#store_or_update_modal .modal-title').html(
                                '<i class="fas fa-edit"></i> <span>Edit ' + data.id +
                                '</span>');
            ...
            $(document).on('click', '.delete_data', function() {
                ...
                let url = "{{ route('facilitysettings.delete') }}";
            ...
            function multi_delete() {
                ...
                if (ids.length == 0) {
                    ...
                    let url = "{{ route('facilitysettings.bulk.delete') }}";
            ...
            $(document).on('click', '.change_status', function() {
                ...
                let name = $(this).data('name');
                let row = table.row($(this).parent('tr'));
                let url = "{{ route('facilitysettings.change.status') }}";
